...
RSpec.describe 'join', type: :system do
    ...
    let!(:user_name2) { coach_name }
    let!(:email2) { Faker::Internet.email }
    let!(:password2) { Faker::Alphanumeric.alphanumeric(number: 10) } # лимит - 1
    let!(:coach_user_data) { { name: user_name2, email: email2, password: password2} }
    let!(:coach_user) { User.create!(coach_user_data) }
    ...
    before do
      ...
      # p coach_name
      Coach.create!(act: Act.first, name: coach_name, user: coach_user)
    ...
    scenario 'limited' do
        ...
        visit new_user_session_path # входим как тренер
        find("input[placeholder='Email']").set email2
        find("input[placeholder='Пароль']").set password2
        click_button('Войти в аккаунт')

        sleep(1)    
        find('#timetable').click 
        expect(page).to have_text('Записаны: 1/1')  
        click_button(my_time)
        click_button('Удалить')
        click_button('OK')
        expect(page).to have_text('Записаны: 0 /1') 
      end
end