...
module JoinHelper
  ...
  def before_club
    [:club].each { |key| $record.delete(key) if $record&.key?(key) }
    return unless act_params[:activity].nil? && !$record&.key?(:activity) || (Coach.find_by(user_id: current_user&.id).present? || Employee.find_by(user_id: current_user&.id).present?) && current_user.present?
    redirect_to join_path
  end

  def before_calendar
    return unless act_params[:club].nil? && (!$record&.key?(:activity) ||
        !$record&.key?(:service)) && !redir || (Coach.find_by(user_id: current_user&.id).present? || Employee.find_by(user_id: current_user&.id).present?) && current_user.present?
